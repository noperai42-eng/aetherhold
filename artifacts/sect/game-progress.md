...
`npm run dev` → `http://localhost:5063/?pocket=hollow`. That is the way in for a
player: the top-bar button was taken out at review, because this slice is
explicitly not claimed as ship and a permanent button in a shipped game's chrome
says otherwise. `window.aetherhold.enterHollow()` still works from the console,
which is what the shoot uses. Frames: `node scripts/look/hollow.mjs`.
...
- New at review: `tests/hollow-pocket.test.ts`. The seven tests above all passed
  on a hollow whose floor, rim and water were wound inside out and therefore not
  drawn at all — they cannot tell a pocket with terrain from a pocket with none.
  That file counts which way the normals point, and covers the two things the
  pocket borrows from the running game and has to give back: the quality preset,
  and the shadow-casting light.
