...
export function disposeHollowKit(kit: HollowKit): void {
  kit.group.traverse((o) => {
    ...
    // The sun casts, and a shadow-casting light owns a 1024x1024 render target
    // that `scene.remove` does not free — only `Light.dispose` reaches it. This
    // walk used to test `isMesh` alone, and a light is not a mesh, so every trip
    // into the hollow and back left one behind: measured 3 textures to 15 over
    // twelve visits, about seven megabytes of video memory each. It does not
    // show up on Low, where shadows are off entirely, which is why it survived.
