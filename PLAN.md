...
- The invariant floor (`LOOM.md`) holds: `grid.isSolid` is the only collision gate, `moveWithCollision` the only writer of `animPhase`, the controller never scans `world.buildings`, the sim advances only in `TICK_DT` steps and the renderer writes no sim state, no asset enters the repo, no physics engine.
