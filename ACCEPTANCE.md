...
Last run — 2026-09-14, the build-rate round: two counters in `src/eval/run.ts` and four
...
| The mood row never reads `nothing fun to do` on a settler who is not idle | **not held, and now measured.** `needs.ts:553-556` asks exactly one question — `pawn.jobId === null` — to choose between `nothing fun to do` and `tired of working`, and the sim clears `jobId` for reasons that have nothing to do with a settler being idle. `tests/mood-label.test.ts` drives a real colony, seed `20260801`, and finds two shapes of it on the same tracked settler: the ordinary `ASSIGN_INTERVAL` gap between one job finishing and the next being handed out shows the row 293 times over a day, and every one of those ticks `isIdlePawn` (`src/eval/run.ts:403`, the literal predicate `3a-sim-pin-build-rate` pinned for the grid) agrees the settler really is idle — not the sighting the player quoted. Drafting one mid-job is: `setDrafted` (`src/sim/orders.ts:374-389`) cancels the job and clears `jobId` the instant the player presses T, and `tick.ts:316` skips the whole job/idle pass for as long as `drafted` holds, so nothing in the sim ever sets `jobId` again. The row reads `nothing fun to do` for all 4,800 ticks of that drafted day, and `isIdlePawn` — which excludes `pawn.drafted` on line `run.ts:405`, on purpose — says not-idle on every one of them. This is the branch: a settler stood at the wall, drafted, reads `nothing fun to do` in their own mood breakdown the whole time they are on watch. Manual: draft a settler on `:5063` (T) mid-raid, open their card, and read `why that mood` — the row is still there. Not yet a `PLAYTEST.md` step: this round's declared write set is `tests/mood-label.test.ts`, `ROUND_NOTES.md` and `ACCEPTANCE.md` only, so the numbered step lands with the fix. The fix itself is `3e-fix-label`'s, per `PLAN.md`; this round reproduces and names the branch, nothing under `src/` changes, and the fingerprint does not move |
