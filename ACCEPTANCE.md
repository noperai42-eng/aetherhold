...
| A settler has a chin and sideburns, and faces differ from one settler to the next | `makeHead()` in `src/client/render/pawns.ts` reshapes the head sphere below the mouth: a narrower jaw and a chin about 1.5 cm forward, with the same vertex and triangle counts and nothing above y = -0.03 moved. The crop and swept hems have sideburns. The hair shader adds a sheen band and variation between locks. `faceTraitsOf` in `face.ts` deals a moustache or stubble from the beard's bits plus bit 18 (nobody gets two kinds), freckles to one in five from a hash of the seed, and age lines to the grey-haired. Every face has an eye socket, a smile fold and a shadow under the lip, and the beard's top edge is curved. No triangles were added. Deliberate re-pins in `tests/forge-recipes.test.ts`: `head` (hash), `hair` and `hairSwept` (hash and box). Not done: the lower face is dark under the scene light, freckles and stubble are faint on dark skin, and the marks don't export to glTF | automated — `tests/face.test.ts` (traits are dealt without doubling up and the bearded keep their beards, freckle fraction, the chin comes forward and the upper head is unchanged, every settler's face wears its seed's traits), `tests/forge-recipes.test.ts`, `tests/head-read.test.ts`, `tests/lighting.test.ts`, full suite; frames — `.look/shots/r31/` (`f-*`, `t-*`, `u-*`) against `r31-before/` |
| A settler's eyes sit in the face instead of standing off it | The eye bead is pressed into a lens 5 mm deep (`settler.eye`, the same 42 vertices) and seated by `EYE_SEAT` in `src/client/render/pawns.ts` along the skull's normal, with its centre 2.3 mm under the skin and the left eye mirrored. `EYE_GLSL` in `face.ts` paints the iris towards the nose and up and raises the lid, so the gaze stays forward. No triangles were added. Deliberate re-pin: `settler.eye` in `tests/forge-recipes.test.ts`. The r31 dark lower face was the capture harness's lighting, not the game's, so no lighting changed. Not done: the lid reads as a small skin-coloured cap close up, and glTF export carries the plain lens | automated — `tests/head-read.test.ts` (every vertex of both lenses stands under 6 mm off the skull and the front clears it by 1 mm; fails on the old bead at 21.7 mm), `tests/forge-recipes.test.ts`, `tests/lighting.test.ts`, `tests/face.test.ts`, full suite; frames — `.look/shots/r32/` (`f-*`, `u-*`, game light) against `r32-before/` |
| A settler's hair reads as locks of strands growing from a crown, not a striped helmet | `HAIR_GLSL` in `src/client/render/face.ts`: the whorl is behind the crown, 48 strands each with a hashed shade, locks of four with shadowed gaps, all wrapped so the back seam is invisible; `HAIR_BUMP_GLSL` raises the same strands in relief after three's normal code. One program for every head of hair; no triangles added, no goldens moved. Not done: the whorl is a small star seen from straight above and behind, the silhouette is still a smooth shell, and glTF export carries no strands | automated — `tests/face.test.ts` (the relief is declared, set in the colour pass, and applied after `normal_fragment_maps` and before lighting; strand and lock ids wrap at 48 and 12; one shared hair program), full suite; frames — `.look/shots/r33/` (`h-*`, `f-*`, game light, including the manager-distance `f-game`) against `r33-before/` |
