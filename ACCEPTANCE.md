...
| Every bench frame stands on the turf it is photographed against | **not held, and now measured.** The wood's frame looks past the edge of the bench's own ground. `stage.ts` lays a 200 m square under the subject to catch its shadow and say which way is down, and `fitDistance` stands the camera on the line out of the subject's centre — so a wood four metres tall at its middle rides the camera up with it while the pitch stays 27 degrees down, and the two top corners of the frame land 105 m from the origin against a turf that stops at 100. A corner ray carries the horizontal half field as well as the vertical one and leaves along the diagonal, where a square plane's edge is nearest; the middle of that same top edge is still turf at 71 m, which is why the rim shows in the corners and nowhere else. A third of the top quarter of that picture is sky, and it was the same third at four to a row, so this is not the counts. `tests/forge-stage.test.ts` writes out how far all six frames reach — 35, 11, 105, 21, 25, 43 — and records that the wood is the one over and the next-widest is 43 m short of the edge. Widening the plane changes what is behind every tree in every frame two rounds are compared across, so it is a look-loop round with frames and the brief is in [FORGING.md](FORGING.md) |
